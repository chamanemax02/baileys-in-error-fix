...
import { aesDecryptGCM, aesEncryptGCM, hmacSign } from './crypto.js';
...
export function encryptPollVote({ selectedOptions }, { pollCreatorJid, pollMsgId, pollEncKey, voterJid }) {
    const sign = Buffer.concat([
        toBinary(pollMsgId),
        toBinary(pollCreatorJid),
        toBinary(voterJid),
    ...
    const encPayload = aesEncryptGCM(payload, decKey, encIv, aad);
    return {
        encPayload,
        encIv
    };
