...
export async function buildTcTokenFromJid({ authState, jid, baseContent = [], getLIDForPN }) {
    try {
        ...
        baseContent.push({
            tag: 'tctoken',
            attrs: {
                t: entry?.timestamp ? String(entry.timestamp) : undefined
            },
...
export async function storeTcTokensFromIqResult({ result, fallbackJid, keys, getLIDForPN, getPNForLID, onNewJidStored }) {
    ...
    for (const tokenNode of tokenNodes) {
        ...
        await keys.set({ tctoken: tokenWrite });
        onNewJidStored?.(storageJid);
        if (altJid) {
            onNewJidStored?.(altJid);
        }
    }
}
