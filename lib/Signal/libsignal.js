...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc) {
    ...
    const repository = {
        ...
        async decryptMessage({ jid, type, ciphertext }) {
            const doDecryption = async (targetJid) => {
                const addr = jidToSignalProtocolAddress(targetJid);
                ...
                if (type === 'pkmsg') {
                    ...
                    if (identityKey) {
                        ...
                        if (identityChanged) {
                            logger.info({ jid: targetJid, addr: addrStr }, 'identity key changed or new contact, session will be re-established');
                        }
                    }
                }
            ...
            try {
                return await doDecryption(jid);
            }
            catch (err) {
                const isSessionError = err?.message?.includes('No session') || err?.name === 'SessionError';
                if (isSessionError) {
                    let altJid = null;
                    if (isLidUser(jid)) {
                        altJid = await lidMapping.getPNForLID(jid);
                    }
                    else {
                        altJid = await lidMapping.getLIDForPN(jid);
                    }
                    if (altJid) {
                        logger.debug({ jid, altJid }, 'No session record for primary JID, trying alternative JID mapping');
                        try {
                            const result = await doDecryption(altJid);
                            // If successful, migrate the session to primary JID so we don't have to fallback next time!
                            try {
                                await repository.migrateSession(altJid, jid);
                                logger.debug({ from: altJid, to: jid }, 'Successfully migrated session to primary JID');
                            }
                            catch (migrationErr) {
                                logger.debug({ migrationErr: migrationErr.message }, 'Failed to migrate session');
                            }
                            return result;
                        }
                        catch (altErr) {
