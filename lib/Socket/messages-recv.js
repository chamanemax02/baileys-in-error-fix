...
export const makeMessagesRecvSocket = (config) => {
    ...
    const sendRetryRequest = async (node, forceIncludeKeys = false, decryptionError = '') => {
        ...
        const isSessionCorrupted = decryptionError && (
            decryptionError.toLowerCase().includes('session') ||
    ...
    const handleMessage = async (node) => {
        ...
        if (tcTokenNode && tcTokenNode.content instanceof Uint8Array) {
            ...
            if (rawJid && !isJidGroup(rawJid) && !isJidNewsletter(rawJid) && !isJidStatusBroadcast(rawJid)) {
                try {
                    ...
                    if (incomingTs) {
                        ...
                        if (existingTs === 0 || incomingTs >= existingTs) {
                            ...
                            let altJid = null;
                            if (isLidUser(storageJid)) {
                                altJid = await getPNForLID(storageJid);
                            }
                            else {
                                altJid = await getLIDForPN(storageJid);
                            }
                            if (altJid) {
                                tokenWrite[altJid] = tokenWrite[storageJid];
                            }
                            await authState.keys.set({ tctoken: tokenWrite });
                            trackTcTokenJid(storageJid);
                            if (altJid) {
                                trackTcTokenJid(altJid);
                            }
                            logger.debug({ jid: storageJid, altJid, timestamp: incomingTs }, 'extracted and saved incoming tctoken from message');
                        }
                    }
                }
                catch (err) {
                    logger.warn({ err: err?.message, jid: rawJid }, 'failed to save incoming tctoken');
                }
            }
        }
            ...
            if (attrs.error === SERVER_ERROR_CODES.MessageAccountRestriction) {
                ...
                if (ackFrom) {
                    try {
                        ...
                        const existingTcData = await authState.keys.get('tctoken', [tcStorageJid]);
                        const existingEntry = existingTcData[tcStorageJid];
                        const cleared = existingEntry?.senderTimestamp !== undefined
                            ? { token: Buffer.alloc(0), senderTimestamp: existingEntry.senderTimestamp }
                            : null;
                        await authState.keys.set({ tctoken: { [tcStorageJid]: cleared } });
                        logger.debug({ from: ackFrom }, 'cleared stale/expired tctoken due to error 463');
                    } catch (err) {
                        logger.debug({ from: ackFrom, err: err?.message }, 'failed to clear stale tctoken');
                    }
                }
                if (ackFrom && !inFlight463Recoveries.has(ackFrom)) {
                    ...
                    void (async () => {
                        try {
                            ...
                            logger.warn({ from: ackFrom, err: err?.message }, 'failed 463 token recovery issuance');
                        }
                        finally {
